...
public:
    ...
    bool tickManual()
    {
        ...
        case 2: // пауза
...
#else
                if ((steps + so1) >= prfS[GS_FAST_PROFILE - 1])
                    us = usMin;
                else
                {
                    int j = 0;
                    while ((steps + so1) >= prfS[j])
                        j++;
                    us = prfP[j];
                }
#endif
            ...
            { // торможение
                ...
                us = constrain(us, usMin, us0);
#else
                if ((S - steps) >= prfS[GS_FAST_PROFILE - 1])
                    us = usMin;
                else
                {
                    int j = 0;
                    while ((S - steps) >= prfS[j])
                        j++;
                    us = prfP[j];
                }
#endif
        ...
        case 4: // плавная остановка
            ...
            us = constrain(us, usMin, us0);
#else
            if (stopStep >= prfS[GS_FAST_PROFILE - 1])
                us = usMin;
            else
            {
                int j = 0;
                while (stopStep >= prfS[j])
                    j++;
                us = prfP[j];
            }
#endif
            if (pos == tar || stopStep <= 0 || us >= us0)
                brake();
            return status;
        }
#else
    ...
    {
        if (setSpeed((int32_t)speed))
            us = 1000000.0 / abs(speed);
    }
    ...
    void setTarget(int32_t ntar, GS_posType type = ABSOLUTE)
    {
        ...
        if (ss > S || (status && ndir != dir))
        { // не успеем остановиться или едем не туда
            revF = 1;
            bufT = tar;
            tar = pos + ss * dir;
            S = ss;
        ...
        { // ускорение задано и мин. скорость выше порога
            if ((int32_t)V * V / a - ((int32_t)v1 * v1 / a >> 1) > S)
            { // треугольник
                if (revF)
                    s1 = 0;
                else
    ...
    void setAcceleration(uint16_t acc)
    {
        ...
        na = acc;
        if (!status)
        { // применяем, если мотор остановлен
            a = na;
            if (a != 0)
                us0 = 0.676 * 1000000 * sqrt(2.0 / a);
            else
                us0 = usMin;
            changeSett = 0;
            calcPlan();
            ...
            changeSett = 1; // иначе флаг на изменение
#endif
    }
    ...
    {
#ifndef GS_NO_ACCEL
        if (a == 0 || us > GS_MIN_US || status == 3 || !status)
